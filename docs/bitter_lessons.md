...
**Where it bit:** the prior fp8 parity probes (post-loop-04 tuning attempts) and loop-07
(wide-PV). Risk is either tuning a dead end forever, or deferring with no evidence (which strict
review treats as incomplete).
...
**Where it bit:** loop-06/07. The default-path change (wide QK: 8 narrow `32x32x16` → 2 wide
`32x32x64` MFMAs for QK^T) cut QK MFMA count ~4× but moved wall-clock only ~5–9%; a separately
tested wide-PV variant cut PV MFMA ~4× yet ran *slower* than the default. The kernel was not
MFMA-issue-bound: roofline showed HBM ~8–10% utilized, ~41% of fp8 MFMA peak; the real constraint
was a VGPR-limited dual-wave pipeline (occupancy/latency-bound).

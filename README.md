...
| [`06`](results/loop-06-fp8-fa-vs-aiter-asm-rocprofv3.md) | `ANALYSIS`: rocprofv3 comparative deep-dive identified a **4× MMA-width mismatch** — aiter uses `32x32x64` (65536 MACs/op), FlyDSL `32x32x16` (16384). | Revised the earlier "purely stall-bound, MMA ~8%" framing: the stalls are downstream of issuing 4× as many MFMAs. Produced the concrete, testable hypothesis (adopt the wide atom) executed in loop 07 — which showed width is one lever (+5–9% via wide QK), not the whole gap (non-causal remains VGPR/occupancy-bound). |
| [`07`](results/loop-07-fp8-fa-wide-mfma-gfx950.md) | `IMPROVEMENT` (T1): wide `32x32x64` QK MFMA default-on (+5–9%); on a 60-config sweep **causal beats aiter native fp8 ASM (geomean 117%, up to 151%)**, non-causal trails (geomean 68%). Delivered to PR [`ROCm/FlyDSL#711`](https://github.com/ROCm/FlyDSL/pull/711); gate-off ISA byte-identical. | Executed loop 06's hypothesis. Single-point benchmarks had hidden that causal already wins; the remaining non-causal gap is a VGPR/occupancy structural wall, not instruction width (wide-PV levers tested-negative). |
...
- [`docs/bitter_lessons.md`](docs/bitter_lessons.md) (reusable methodology + technical lessons distilled across loops)
